...
def testView(request):  
    ...
    return render(request, 'Dashboard/addTest.html',context)


@login_required(login_url='loginView') 
def testdetailView(request,id):  
    patient = Profile.objects.filter(id=id) 

  
    if request.method == 'POST': 
        text = request.POST.get('text') 
        Profile.objects.filter(id=id).update(bio=text) 
    ...
    return render(request, 'Dashboard/addTest2.html',context)